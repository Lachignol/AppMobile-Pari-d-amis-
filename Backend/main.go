...
func init() {
	...
	oregonTime := utcNow.Add(-8 * time.Hour) // Adapte cette ligne pour UTC-7 si c'est l'heure d'été (PDT)
	...
	fmt.Printf("Oregon Time: %02d:%02d (UTC-%d)\n", oregonHour, oregonMinute, 8) // Ajuste à 7 pour l'heure d'été
	...
	myScheduler.Every(1).Wednesday().At("02:37").Do(scheduler.GetMatchAndSaveThemInJson)
